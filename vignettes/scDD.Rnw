...
Verify that this object is a member of the \Rclass{ExpressionSet} class and 
that it contains 200 samples and 30 genes.  The \Rcode{phenoData} slot 
(which contains a matrix of metadata for the cells) should have a column 
...
input gene set needs to be in \Rclass{ExpressionSet} format, and should 
...
condition or grouping variable in the \Rcode{phenoData} slot is named 
...
The \Robject{RES} object is a list with four items: the first is a data 
frame with nine columns: 
\begin{enumerate}
  ...
  condition 2 alone
\end{enumerate}
  The remaining three elements are data frames (first for condition 1 and 2
  combined, then condition 1 alone, then condition 2 alone) that contains 
  the cluster memberships for each sample (cluster 1,2,3,...) in columns 
...
<<main engine perm>>=
RES_perm <- scDD(scDatExSim, prior_param=prior_param, 
...
\Rpackage{BiocParallel} package.  By default, \Rpackage{BiocParallel} will 
use all cores on the machine, but you can optionally specificy the number of 
cores to use by changing the \Rcode{param} argument in the \Rcode{scDD} 
function call (the \Rcode{param} argument is an object of class 
\Rcode{MulticoreParm} of the \Rpackage{BiocParallel} package).  
For example, to use 12 cores, specify 
\Rcode{param=MulticoreParam(workers=12)}.

The results object returned by \Rfunction{scDD} remains exactly as described 
...
of the \Rclass{ExpressionSet} class.  For more detailed instructions, refer 
to the \Rpackage{Biobase} package documentation.

Here we will convert the simulated data object \Robject{SD} returned by 
\Rfunction{simulateSet} (from the previous section) into an 
\Rclass{ExpressionSet} object. First, load the \Rpackage{Biobase} package:
...
Once our labeling is intact, we can call the \Rfunction{ExpressionSet} 
...
set the \Robject{zero.thresh} argument to 1 so that genes are filtered out if 
they are all zero, and we set the \Robject{median_norm} argument to
\Rcode{FALSE} to return raw data (no normalization is carried out).
...
threshold on the proportion of zeroes and apply median normalization.  In this
...
\Robject{median_norm} argument to \Rcode{TRUE} to return median normalized
counts.
...
Next, load the Biobase package to facilitate subset operations on 
ExpressionSet class objects:
