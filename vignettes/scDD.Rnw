...
\begin{enumerate}
  ...
  \item \Rcode{nonzero.pvalue.adj}: Benjamini-Hochberg adjusted permutation p-value for testing of independence of condition membership with clustering
...
The option to use the Kolmogorov-Smirnov test instead of the full permutation testing procedure is invoked by setting the number of permutations to zero (the \Rcode{permutations} argument) when calling the main \Rfunction{scDD} function, as follows.  
