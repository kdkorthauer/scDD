...
<<eval=TRUE, print=TRUE, results=hide>>=
library(scDD)
@
...
<<eval=TRUE, print=TRUE, results=verbatim>>=
class(scDatExSim)
show(scDatExSim)
@
...
<<eval=FALSE, print=TRUE, results=hide>>=
...
<<eval=FALSE, print=TRUE, results=tex>>=
...
<<DDplots, fig=TRUE, include=FALSE, print=FALSE, echo=FALSE, width=8.5, height=11>>=
library(gridExtra)
# plot side by side violin plots for Gene 1 (DE)
